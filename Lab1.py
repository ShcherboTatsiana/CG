...
def hls_to_rgb(h, l, s):
    ...
    #Define r, g, b depending on the sector of the color wheel
    if 0 <= h < 60:
        r, g, b = c, x, 0
    elif 60 <= h < 120:
        r, g, b = c, x, 0
...
def rgb_to_cmyk(r, g, b):
    ...
    # Convert CMY values to percentages
    c = round(c * 100)
    m = round(m * 100)
    y = round(y * 100)
    k = round(k * 100)
...
# Allows to choose color on circle
...
c_slider = tk.Scale(cmyk_frame, from_=0, to=100, orient=tk.HORIZONTAL, command=on_cmyk_slider_change)
c_slider.grid(row=0, column=1)
c_spinbox = tk.Spinbox(cmyk_frame, from_=0, to=100, increment=1, width=3, command=on_cmyk_spinbox_change) 
c_spinbox.grid(row=0, column=2, padx = 10)

tk.Label(cmyk_frame, text="M:  0-100").grid(row=1, column=0, padx = 10)
m_slider = tk.Scale(cmyk_frame, from_=0, to=100, orient=tk.HORIZONTAL, command=on_cmyk_slider_change) 
m_slider.grid(row=1, column=1)
m_spinbox = tk.Spinbox(cmyk_frame, from_=0, to=100, increment=1, width=3, command=on_cmyk_spinbox_change) 
m_spinbox.grid(row=1, column=2, padx = 10)

tk.Label(cmyk_frame, text="Y:  0-100").grid(row=2, column=0, padx = 10)
y_slider = tk.Scale(cmyk_frame, from_=0, to=100, orient=tk.HORIZONTAL, command=on_cmyk_slider_change)
y_slider.grid(row=2, column=1)
y_spinbox = tk.Spinbox(cmyk_frame, from_=0, to=100, increment=1, width=3, command=on_cmyk_spinbox_change) 
y_spinbox.grid(row=2, column=2, padx = 10)

tk.Label(cmyk_frame, text="K:  0-100").grid(row=3, column=0, padx = 10)
k_slider = tk.Scale(cmyk_frame, from_=0, to=100, orient=tk.HORIZONTAL, command=on_cmyk_slider_change) 
k_slider.grid(row=3, column=1)
k_spinbox = tk.Spinbox(cmyk_frame, from_=0, to=100, increment=1, width=3, command=on_cmyk_spinbox_change) 
...
r_slider = tk.Scale(rgb_frame, from_=0, to=255, orient=tk.HORIZONTAL, command=on_rgb_slider_change) 
r_slider.grid(row=0, column=1)
r_spinbox = tk.Spinbox(rgb_frame, from_=0, to=255, increment=1, width=3, command=on_rgb_spinbox_change) 
r_spinbox.grid(row=0, column=2, padx=10)

tk.Label(rgb_frame, text="G:  0-255").grid(row=1, column=0, padx = 10)
g_slider = tk.Scale(rgb_frame, from_=0, to=255, orient=tk.HORIZONTAL, command=on_rgb_slider_change) 
g_slider.grid(row=1, column=1)
g_spinbox = tk.Spinbox(rgb_frame, from_=0, to=255, increment=1, width=3, command=on_rgb_spinbox_change) 
g_spinbox.grid(row=1, column=2, padx=10)

tk.Label(rgb_frame, text="B:  0-255").grid(row=2, column=0, padx = 10)
b_slider = tk.Scale(rgb_frame, from_=0, to=255, orient=tk.HORIZONTAL, command=on_rgb_slider_change) 
b_slider.grid(row=2, column=1)
b_spinbox = tk.Spinbox(rgb_frame, from_=0, to=255, increment=1, width=3, command=on_rgb_spinbox_change)
...
h_slider = tk.Scale(hls_frame, from_=0, to=360, orient=tk.HORIZONTAL, command=on_hls_slider_change)
h_slider.grid(row=0, column=1)
h_spinbox = tk.Spinbox(hls_frame, from_=0, to=360, increment=1, width=3, command=on_hls_spinbox_change)
h_spinbox.grid(row=0, column=2, padx=10)

tk.Label(hls_frame, text="L:  0-100%").grid(row=1, column=0, padx = 20)
l_slider = tk.Scale(hls_frame, from_=0, to=100, orient=tk.HORIZONTAL, command=on_hls_slider_change)
l_slider.grid(row=1, column=1)
l_spinbox = tk.Spinbox(hls_frame, from_=0, to=100, increment=1, width=3, command=on_hls_spinbox_change) 
l_spinbox.grid(row=1, column=2, padx=10)

tk.Label(hls_frame, text="S:  0-100%").grid(row=2, column=0, padx = 20)
s_slider = tk.Scale(hls_frame, from_=0, to=100, orient=tk.HORIZONTAL, command=on_hls_slider_change)
s_slider.grid(row=2, column=1)
s_spinbox = tk.Spinbox(hls_frame, from_=0, to=100, increment=1, width=3, command=on_hls_spinbox_change) 
